...
class CMISRepositoryWrapper
{
    ...
    static function processTemplate($template, $values = array ())
    {
        ...
        if (is_array($values))
        {
            ...
            { 
				if ($name != 'CONTENT') {									//Evito activamente el tag del binario que va en base64
					$value = utf8_e_seguro($value);
				}
        ...
        //return mb_ereg_replace("/{[a-zA-Z0-9_]+}/", "", $retval, 'p');	//Deja tags pendientes.. analizar a futuro posibles complicaciones por multibyte
    }
    ...
    static function extractObjectFromNode($xmlnode)
    {
		  ...
		  foreach ($renditions as $rendition) {
            ...
            foreach ($rend_nodes as $rend){
              if ($rend->localName != NULL){
	            $renditionArray[$i][$rend->localName] = utf8_d_seguro($rend->nodeValue);
              }
            }
            $i++;        
	      }
        ...
        foreach ($prop_nodes as $pn)
        {
        	if ($pn->attributes) {
				...
				@$retval->properties[$pn->attributes->getNamedItem("propertyDefinitionId")->nodeValue] = utf8_d_seguro($pn->getElementsByTagName("value")->item(0)->nodeValue);
			}
        }
        
        $retval->uuid = utf8_d_seguro($xmlnode->getElementsByTagName("id")->item(0)->nodeValue);
        $retval->id = utf8_d_seguro($retval->properties["cmis:objectId"]);
        //TODO: RRM FIX THIS
    ...
    static function extractTypeDefFromNode($xmlnode)
    {
        ...
        foreach ($result as $node)
        {
            ...
            {
                $retval->attributes[$node->nodeName] = utf8_d_seguro($node->nodeValue);
            }
            $retval->id = utf8_d_seguro($retval->attributes["cmis:id"]);
        }
        //TODO: RRM FIX THIS
        $children_node = $xmlnode->getElementsByTagName("children");
        if (is_object($children_node)) {
    ...
    static function extractWorkspaceFromNode($xmlnode)
    {
        ...
        foreach ($result as $node)
        {
            $retval->repositoryInfo[$node->nodeName] = utf8_d_seguro($node->nodeValue);
        }
        ...
        foreach ($result as $node)
        {
            ...
            {
                array_push($values, utf8_d_seguro($value->nodeValue));
            }
